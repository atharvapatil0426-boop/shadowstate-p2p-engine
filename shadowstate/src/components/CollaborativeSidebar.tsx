...
export const CollaborativeSidebar: React.FC = () => {
  ...
  const { connectedPeers, isMeshConnected } = useP2PSync(ydoc);
        ...
        <div className="space-y-3 bg-slate-950/60 p-4 rounded-xl border border-slate-800">
          <div className="flex items-center justify-between">
            <span className="text-sm text-slate-300 flex items-center gap-2">
              {isMeshConnected ? <Wifi size={16} className="text-emerald-400" /> : <WifiOff size={16} className="text-rose-400" />}
              Mesh Channel:
            </span>
            <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isMeshConnected ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'}`}>
              {isMeshConnected ? 'Active' : 'Offline Mode'}
            </span>
          </div>
